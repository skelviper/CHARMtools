...
def cli(args):
    """
    CLI function for tdg2pairs command
    """
    tdgfilepath, pairsfilepath, distance = args.input_file, args.output_file, args.distance
    tdg2pairs_main(tdgfilepath, pairsfilepath, distance)

def tdg2pairs_main(tdgfilepath, pairsfilepath, distance):
    start_time = time.time()

    sys.stderr.write("Reading 3dg file\n")
    tdg = read3dgfile(tdgfilepath)
    ...
    df.to_csv(pairsfilepath,sep="\t",index=False,header=None)
...
if __name__ == "__main__":
    ...
    tdg2pairs_main(args.tdgfilepath, args.pairsfilepath, args.distance)
